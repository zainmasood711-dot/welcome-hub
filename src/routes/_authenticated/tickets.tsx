...
import { getPhase2References, listTickets, saveTicket } from "@/lib/phase2.functions";
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <form className="space-y-3" onSubmit={submit}>
            <div className="space-y-2"><Label>العميل</Label><Select value={form.customer_id} onValueChange={(v) => setForm((p) => ({ ...p, customer_id: v }))}><SelectTrigger><SelectValue placeholder="اختر عميل" /></SelectTrigger><SelectContent>{(refs?.customers ?? []).map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
